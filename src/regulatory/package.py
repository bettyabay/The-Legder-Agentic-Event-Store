...
def _build_narrative(
    ...
    for event in events:
        ...
        if event.event_type == "ApplicationSubmitted":
            requested_amount = _to_float(p.get("requested_amount_usd"))
            lines.append(
                f"  [{ts}] Application submitted by applicant '{p.get('applicant_id')}' "
                f"requesting ${requested_amount:,.0f} for '{p.get('loan_purpose')}'."
            )
        elif event.event_type == "CreditAnalysisCompleted":
            confidence = _to_float(p.get("confidence_score"))
            rec_limit = _to_float(p.get("recommended_limit_usd"))
            lines.append(
                ...
                f"recommended limit ${rec_limit:,.0f}."
            )
        elif event.event_type == "FraudScreeningCompleted":
            fraud_score = _to_float(p.get("fraud_score"))
            lines.append(
                f"  [{ts}] Fraud screening completed: score {fraud_score:.2f}, "
