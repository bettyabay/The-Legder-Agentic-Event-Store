...
async def _main() -> None:
    parser = argparse.ArgumentParser(description="Run Week-5 CreditAnalysisAgent for one application.")
    parser.add_argument("--application-id", required=True, help="Application ID, e.g. APEX-0012")
    parser.add_argument("--agent-id", default="credit-agent-1")
    ...
    try:
        ...
        print(f"OK: credit agent completed for {args.application_id}")
    finally:
        await pool.close()
